...
namespace Log2Console.UI
{
    ...
    ///         (see
    ///         <see href="http://msdn.microsoft.com/en-us/library/0zszyc6e.aspx">Application Settings for Windows Forms</see>)
    ...
    ///         <item>
    ///             <description>
    ///                 Instantiate a WindowRestorer supplying values for the window state and position
    ///                 at the time the application was last terminated,
    ///                 which come from persistent application settings.
    ///                 The supplied values are used as a starting point. If the application was minimized, it is restored in a
    ///                 normal state.
    ///                 If it was normal or maximized, it is restored to that same state.
    ///                 If the monitor configuration has changed so that the window is no longer visible, it is moved so that
    ///                 it is visible.
    ///             </description>
    ///         </item>
    ///         <item>
    ...
    ///         <item>
    ///             <description>
    ///                 Wire up the <b>FormClosing</b> event to save the window state and position at the time the application
    ///                 is terminated,
    ...
    public class WindowRestorer
    {
        ...
        public void TrackWindow()
        {
            ...
            if (form.WindowState == FormWindowState.Normal) WindowPosition = form.DesktopBounds;
            if (form.WindowState != FormWindowState.Minimized) WindowState = form.WindowState;
        }
        ...
        private void Restore()
        {
            ...
            else
            {
                ...
                if (WindowPosition != Rectangle.Empty) form.Size = WindowPosition.Size;
            }
        ...
        ///     </para>
        /// </remarks>
        public static void SetSubWindowRelativeLocation(Form targetForm, Form parentForm, Point parentRelativeLocation,
        ...
        public bool MultipleMonitorMaximize()
        {
                ...
                && Screen.AllScreens.Length > 1)
            {
                ...
                else
                {
                    ...
                    var newSize = new Size(endPoint.X - startPoint.X, endPoint.Y - startPoint.Y);
