...
namespace Log2Console.UI.ControlExtenders
{
    ...
    /// </summary>
    internal class Floaty : Form, IFloaty
    {
        #region events
        ...
        public bool HideHandle { get; set; }
        ...
        {
            if (m.Msg == NativeMethods.WM_NCLBUTTONDBLCLK) // double clicked on border, so reset.
            {
                DockFloaty();
            }
        ...
        // this this member
        public new void Show(IWin32Window win) => Show();
        ...
        private void MakeFloatable(DockState dockState, int offsetx, int offsety)
        {
            ...
            NativeMethods.SendMessage(_dockState.Handle.Handle, NativeMethods.WM_LBUTTONUP, IntPtr.Zero, IntPtr.Zero);
            ps.X -= offsetx;
            ps.Y -= offsety;
            ...
            NativeMethods.SendMessage(Handle, NativeMethods.WM_SYSCOMMAND, new IntPtr(NativeMethods.SC_MOVE | 0x02),
                IntPtr.Zero);
        }
        ...
        private void Handle_MouseHover(object sender, EventArgs e) => _startFloating = true;

        private void Handle_MouseLeave(object sender, EventArgs e) => _startFloating = false;
