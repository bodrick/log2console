...
namespace Log2Console.Receiver
{
    ...
    public class MsmqReceiver : BaseReceiver
    {
        ...
        [Category("Configuration")] public bool Transactional { get; set; }
        ...
        {
            get => _bulkProcessBackedUpMessages;
        ...
        public override string SampleClientConfig => "Configuration for NLog:" + Environment.NewLine +
                                                     "<target name=\"queue\" type=\"MSMQ\" layout=\"${log4jxmlevent}\" " +
                                                     Environment.NewLine +
                    ...
                    if (BulkProcessBackedUpMessages)
                    {
                        ...
                        if (all.Length > 0)
                        {
                            ...
                            for (var i = 0; i < numberofmessages; i++)
                            {
                                ...
                                var logMsg = ReceiverUtils.ParseLog4JXmlLogEvent(loggingEvent, "MSMQLogger");
                                logMsg.LoggerName =
                                    $"{QueueName.TrimStart('.')}_{logMsg.LoggerName}";
                                logs[i] = logMsg;
                            }
