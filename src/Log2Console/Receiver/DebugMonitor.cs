...
namespace Log2Console.Receiver
{
    ...
    public static class DebugMonitor
    {
        ...
        /// </summary>
        private static Thread m_Capturer;
        ...
        public static void Start()
        {
            lock (m_SyncRoot)
            {
                ...
                // such a good idea.				
                m_Mutex = new Mutex(false, typeof(DebugMonitor).Namespace, out var createdNew);
                if (!createdNew)
                ...
                m_SharedFile = CreateFileMapping(new IntPtr(-1), ref sa, PageProtection.ReadWrite, 0, 4096,
                    "DBWIN_BUFFER");
        ...
        private static void Capture()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    var ret = WaitForSingleObject(m_ReadyEvent, INFINITE);
        ...
        private static void Dispose()
        {
            ...
            if (m_AckEvent != IntPtr.Zero)
            {
                if (!CloseHandle(m_AckEvent)) throw CreateApplicationException("Failed to close handle for 'AckEvent'");
