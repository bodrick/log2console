...
namespace Log2Console.Receiver
{
    ...
    public class CsvFileReceiver : BaseReceiver
    {
        ...
        public List<FieldType> FieldList { get; set; } = new List<FieldType>
        {
        ...
        private void ReadFile()
        {
            ...
            while ((fields = ReadLogEntry()) != null)
            {
                ...
                if (fields.Count == FieldList.Count)
                {
                ...
                try
                {
                    switch (fieldType.Field)
                    {
                        ...
                        case LogMessageField.TimeStamp:
                            DateTime.TryParseExact(fieldValue, DateTimeFormat, null, DateTimeStyles.None, out var time);
                        ...
                        case LogMessageField.SourceFileName:
                            ...
                            //Detect the Line Nr
                            var fileNameFields = fieldValue.Split(new[]
                            {
                                ":"
        ...
        private List<string> ReadLogEntry()
        {
            ...
            do
            {
                ...
                if (quoteDetected) quoteString.AppendLine();
                    ...
                    //First check for Quote Char in fields
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            quoteString.Append(
                                Delimiter); //Since this is enclosed in the Quote Char's it is part of a string field, and not valid delimiter                            
                        }
                    }
                }

                //If this is a normal log entry, without any quotes, then check that the correct amount of fields is detected
                if (!quoteDetected && finalFields.Count != FieldList.Count)
                {
                    return null;
                }
            } while (finalFields.Count < FieldList.Count
            ); //If this is a multi line log, keep on reading the following lines
    ...
    <column name=""file"" layout =""${callsite:className=false:methodName=false:fileName=true:includeSourcePath=true}"" />
        ...
        public override void Initialize()
        {
            ...
            _fileWatcher = new FileSystemWatcher(path ?? throw new InvalidOperationException(), _filename)
            {
        ...
        private void AutoConfigureHeader()
        {
            ...
            try
            {
                var fieldList = new List<FieldType>();
                foreach (var field in fields)
                {
                    if (UserSettings.Instance.CsvHeaderColumns.Any(f => f.Name == field))
                    {
                        fieldList.Add(UserSettings.Instance.CsvHeaderColumns.SingleOrDefault(f => f.Name == field));
                    ...
                    {
                        fieldList.Add(new FieldType(LogMessageField.Properties, field, field));
                    }
                }
            ...
            {
                MessageBox.Show($"Could not Parse the Header: {line}\n\rError: {ex}", "Error Parsing CSV Header");
            }
        }
